...
//MAKE NODE CLASS, NOT STRUCT. MAKE NIL CONSTRUCTOR.
...
class RBTree {
    ...
    public:
        RBTree() {
            this->head = new Node;
            this->nil = new Node; this->nil->size = 0;
            this->count = 0;
        ...
        //DESTROY EVERYTHING AND THEN DEEP COPY
        RBTree(RBTree &right) {
            this->destroy(this->head);
            delete this->head;
            delete this->nil;
            ...
            head = new Node;
            nil = new Node; nil = right.nil;
            count = right.count;
            cout << "TEST" << endl;
            head = copy(right.head);
        }

        RBTree& operator=(RBTree &right) {
            nil = right.nil;
    ...
    private:
        ...
        int sizeOf(Node* z) {
            ...
            else    return 1 + z->left->size + z->right->size;
        }
        ...
        //need to redo, should pass nil through as parameter?
        Node* copy(Node* x) {
            if (x != this->nil) {
                Node* y = new Node;
                y->key = x->key;
                ...
                y->left = this->copy(x->left);
                y->right = this->copy(x->right);

                return y;
            }
            return this->nil;

            /*if (x == this->nil) return this->nil;
            ...
            if (x->left != this->nil)   y->left = copy(x->left);
            else    y->left = this->nil;
            if (x->right != this->nil)  y->right = copy(x->right);
            else    y->right = this->nil;
        ...
        void destroy(Node* z) {
            ...
            this->destroy(z->left);
            if (z->left != this->nil)   delete z->left;
            this->destroy(z->right);
            if (z->right != this->nil)  delete z->right;
        }
};
